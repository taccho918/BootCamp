...
\end{verbatim}
本手順書では，システムコール\verb|sys_mysyscall()|のシステムコール番号を317と定義する．このファイル内におけるフォーマットは，左から順に\verb|<number> <abi> <name> <entry point>|である．各項目について，以下に詳細を示す．

\newpage
\begin{enumerate}
\item \verb|number|: システムコール番号
\item \verb|abi|: ABI（Application Binary Interface）を表す．ABIとは，アプリケーションのバイナリファイルとカーネル間のインタフェースである．\verb|64|または\verb|x32|，\verb|common|のいずれかを指定する．
